...
This template is not built on Aurelia and does not depend on Aurelia at
runtime. It borrows Aurelia 2's developer experience for game code: plain data
classes, decorators, and typed dependency injection instead of framework-
specific base types and stringly-typed APIs.
...
| Decorators declare intent       | `@component` marks data, `@system({ priority })` records ordering, `@query(...)` wires data   |
| Scoped `resolve()` DI           | Each World constructs its manifest inside a private scope; fields resolve during construction |
| Convention over configuration   | Class fields are the schema; a no-field class is a tag; the lifecycle is the three hooks      |
| Explicit over magic             | Queries list their types (`@query(Position, Velocity)`), never infer them from names          |
...
- Aurelia Kernel itself — the internal container supplies only typed instance
  providers, construction-time resolution, scopes, and disposal. If a project
  later needs child scopes or richer resolvers, adopt `@aurelia/kernel`
  directly; the call sites already use the same vocabulary.
...
- Add behavior: a `@system({ priority })` class with `@query(...)` fields and
  an `execute(dt)`.
...
- Give every World an explicit manifest: `new World({ systems: [Shooting] })`.
- Systems that create entities resolve `IWorld` during construction and retain
  that World-scoped reference; they do not use process-global state in
  `execute()`.
